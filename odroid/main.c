...
typedef enum { GO2_DEVICE_UNKNOWN = 0, GO2_DEVICE_GB, GO2_DEVICE_SGB, GO2_DEVICE_GBC, GO2_DEVICE_GBA } emu_device_t;
...
int main(int argc, char** argv) {
	...
	while (1) {
		...
		if (mCoreSyncWaitFrameStart(&thread.impl->sync)) {
			...
			go2_presenter_post(presenter, surface, 0, 0, 480, 320, 0, (dw - sw) / 2, dh, sw, GO2_ROTATION_DEGREES_270);
